...
class Huffman:
    ...
    def __init__(self, text, size=128):
        self.text = text
        self.dictionary = [None]*size # default size 
    ...
    def calculate_frequency(self):
        ...
        for char in self.text:
            if self.dictionary[self.ord_position(char)] is None:
    ...
    def huffman(self):
        ...
        for i in range(len(self.dictionary)):
            if type(self.dictionary[i]) == int and self.unique_chars == 1:
                self.dictionary[i] = bitarray('1')
            elif self.dictionary[i] is not None:
                bitarray.reverse(self.dictionary[i])

    def encode(self, elem, left):
        for char in elem.key:

            if type(self.dictionary[self.ord_position(char)]) == int:
                self.dictionary[self.ord_position(char)] = bitarray()

            if left:
                self.dictionary[self.ord_position(char)].append(0)
            else:
                self.dictionary[self.ord_position(char)].append(1)
    ...
    def get_code_char(self, char):
        if self.dictionary[self.ord_position(char)] is None:
            assert "encoding not found"
        return self.dictionary[self.ord_position(char)]